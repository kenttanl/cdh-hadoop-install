            ...
            do
                ...
                if [ "$userchoice" == "y" ] || [ "$userchoice" == "Y" ]; then
                    ...
                    do
                        ...
                        ssh $h "sudo cp /etc/hosts /etc/hosts_bak;sudo mv /tmp/hosts /etc/"
                    done
        ...
        # 4. selinux check
            ...
            do
                ...
                if [ "$userchoice" == "y" ] || [ "$userchoice" == "Y" ]; then
                    ...
                    do
                        echo "${bldcyn}********************* $h *********************${txtrst}"
                        ssh $h "sudo sestatus| sed -n '1p';sudo sestatus|sed -n '5p';grep -q 'SELINUX=disabled' /etc/sysconfig/selinux"
                        if [ "$?" != "0"  ]; then
                            ssh $h "sudo sed -i 's/SELINUX=enforcing/SELINUX=disabled/' /etc/sysconfig/selinux;sudo sed -i 's/SELINUX=permissive/SELINUX=disabled/' /etc/sysconfig/selinux;"
                ...
                if [ "$userchoice" == "y" ] || [ "$userchoice" == "Y" ]; then
                    ...
                    do
                        ...
                        if [ "$hugepage1" != "\[never\]" ]; then
                            ...
                            scp -q ./conf/rc.local $h:/tmp/;
                            ssh $h "sudo cp /etc/rc.d/rc.local /etc/rc.d/rc.local.bak; sudo cp /tmp/rc.local /etc/rc.d/; sudo chown root /etc/rc.d/rc.local"
                            ssh $h "sudo chmod u+x /etc/rc.d/rc.local;sudo sysctl -p; sudo systemctl start rc-local"
                        fi
        ...
        # 10. copy files
            ...
            do
                ...
                if [ "$userchoice" == "y" ] || [ "$userchoice" == "Y" ]; then
                    ...
                    do
                        echo "${bldcyn}********************* $h *********************${txtrst}"
                        ssh $h "sudo rm -r /cache/yarn/;sudo chown hbase:hbase /cache;ls -al /cache"
                    done
                    echo "${bldcyn}--------------------- finished! ---------------------${txtrst}"
                    break
                elif [ "$userchoice" == "n" ] || [ "$userchoice" == "N" ]
